...
object ExtractsFromHive {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(!doc.isEmpty()){
      println("Config taken from mongo configuration")
    ...
    sparkConf.registerKryoClasses(Array(Class.forName("anthem.ehub.bds.extracts.ExtractsFromHive"))) //Comt for local
  ...
  def genCmbndExtracts(appConf: Config, sparkContext: SparkContext, sparkConf: SparkConf, sqlContext: SparkSession, args: Array[String], deltaStDt: String, generateFile: Boolean) = {
    ...
    for (doc <- consRecords) {
      ...
      if (doc.getString("Customertype").startsWith("castlight")) {
        ...
        val df = processExtracts(appConf, sparkContext, sparkConf, sqlContext, doc, args, deltaStDt, generateFile)
      }
...
/*    val executor: ExecutorService = Executors.newFixedThreadPool(outputList.length)
    ...
    outputList.map(out => {
      ...
      val out1 = out.toDF()
      if (out != null && out1.take(1).length>0){
        val dataFrame = fixColumns(out1, doc, sqlContext, condiTypes(i), "columnHeaders").distinct()
        if (headerFlag) {
          header = dataFrame.columns.mkString(doc.getString("delimiter"))
          headerFlag = false
        }
  ...
  def processCastlightExtracts(appConf: Config, sparkContext: SparkContext, sparkConf: SparkConf, sqlContext: SparkSession, doc: Document, args: Array[String], deltaStDt: String, genFile: Boolean) = {
    ...
    val HIVE_DATABASE = doc.getString("hive_DB")
    val HIVE_TABLE = doc.getString("hive_table")
    sqlContext.sql(s"use $HIVE_DATABASE")
    println(s"Hive Database $HIVE_DATABASE")
    val genQuery = doc.getString("genQuery").replaceAll("<HIVE_TABLE>",HIVE_TABLE).replaceAll("<INDIC_ID>", concatString(args(4))).replaceAll("<GRP_ID>", concatString(args(5)))
    println("genQuery -- "+genQuery)
    val outputMainDF = sqlContext.sql(genQuery)
    println("Out main query is executed "+ new Date())
    ...
    val remQuery = doc.getString("remQuery").replaceAll("<HIVE_TABLE>",HIVE_TABLE).replaceAll("<INDIC_ID>", concatString(args(4))).replaceAll("<GRP_ID>", concatString(args(5))).replaceAll("<ID_KEY>", concatString(hcidArray))
    println("remaining query after replace -- "+remQuery)
    val reminDF = sqlContext.sql(remQuery)
    ...
    if (reminDF != null && reminDF.take(1).length > 0 &&  flag) {
      ...
      println("remaing df query is executed")
    } else {
  ...
  def processFileGeneration(qryDataFrame: DataFrame, doc: Document, args: Array[String], sparkContext: SparkContext, sqlContext: SparkSession, appConf: Config) {
     ...
     //   var futureList = new scala.collection.mutable.ListBuffer[Future[String]]()
        for (grpBy <- grpByFld) {
         ...
         if (finalDataFrame != null && grpBy != null) {
          ...
          /*  generateFile(finalDF, path + "/" + grpBy, mergeLocation, fileName, finalLoc, doc.getString("delimiter"), sparkContext)
            futureList += executor.submit(new Callable[String]() {
              def call() = {
                println("Printed")*/
           // println("dataframe count -- "+finalDF.count +"fileNAme -- "+fileName + "merge location -- "+mergeLocation)
               generateFile(finalDF, path + "/" + grpBy, mergeLocation, fileName, finalLoc, doc.getString("delimiter"), sparkContext)
                //fileN
             /* }
            })*/
   ...
   /*     executor.shutdown()
        ...
        val fileName = "Anthem_" + csType + "_" + dateFolder
        generateFile(qryDataFrame, path, mergeLocation, fileName + ".csv", finalLoc, doc.getString("delimiter"), sparkContext)
      }
    }
  }

  def generateFile(finalDataFrame: DataFrame, path: String, mergeLocation: String, fileName: String, finalLoc: String, delimiter: String, sparkContext: SparkContext) = {
    Utils.writeDataToCSV(finalDataFrame, sparkContext, path, mergeLocation, fileName, false, null, null, delimiter, "")
    val header = finalDataFrame.columns.mkString(delimiter)
    val finalLocation = mergeLocation + "/merged"
    Utils.copyFromHDFSToLocalFile(sparkContext, mergeLocation + "/" + fileName, finalLoc + "/" + fileName, header, "")
