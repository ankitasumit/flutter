...
object OtherExtracts10 extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf()
      ...
      .setExecutorEnv("num-executors", "10")
      .setExecutorEnv("executor-cores", "1")
      .setExecutorEnv("executor-memory", "10G")
    ...
    sparkConf.registerKryoClasses(Array(Class.forName("anthem.ehub.bds.extracts.OtherExtracts10"))) //Comt for local
...
//    println("Coll Name " + doc.get("collectionName")  +"    "+ doc.get("collectionName") )
...
//    val genData = MongoSpark.load(sparkContext, readConfig).withPipeline(Seq(Document.parse(genblk)))
    ...
    val outputList = condiTypes.map(condiType => MongoSpark.load(sparkContext, readConfig).withPipeline(
      List(
    ...
    println("condiTypes		"+ condiTypes.mkString(","))
    ...
    outputList.map(out => {
        i = i + 1;
        val condi = condiTypes(i)
        println("condi  " + condi)
      futureList += executor.submit(new Callable[String]() {
        def call() = {
          println("Printed   " + condi  + "    "+ i)
          val dataFrame = fixColumns(out.toDF(), doc, sqlContext, condi, "columnHeaders")
          if (headerFlag) {
            header = dataFrame.columns.mkString(doc.getString("delimiter"))
            headerFlag = false
          }
          writeDF(dataFrame, condi, extractType, sparkContext, dateFolder, doc)
//          i = i + 1;
          println("Before response  "+ condi)
          condi
        }
      })
    })
    ...
    outputList.map(out => {
      ...
      val outDF = out.toDF()
      if (out != null && !outDF.limit(1).collect().isEmpty ) { //&& out.toDF().take(1).length>0
        val dataFrame = fixColumns(outDF, doc, sqlContext, condiTypes(i), "columnHeaders")
        if (headerFlag) {
          header = dataFrame.columns.mkString(doc.getString("delimiter"))
          headerFlag = false
        }
        println("Count dataFrame i  "+ i +"   "+ condiTypes(i) +"  "+ dataFrame.count())
...
//    if(dataFrame !=null && dataFrame.take(1).length>0){
//    if (!dataFrame.limit(1).collect().isEmpty) {
      println("came inside not null check in writedF")
      if ("DELTA".equalsIgnoreCase(loadType)) {
        ...
        println("Came inside else blk of writeDF")
...
//    println("Passed Here genMatchBlockAll")
    ...
    var hcidArray: List[String] = List[String]() //new scala.collection.mutable.ListBuffer[String]()
    var outputMainDF: Dataset[SchemaData] = null;
    var outputMainBool:Boolean = false
    ...
    //var filepath = "/dv/hdfsdata/vs2/ehb/bds1/phi/no_gbd/r000/outbound/Extracts/CastLight/data"
    var filepath = doc.getString("remTempPath") 
    
    
    var i = 0
    var staticount = 20000
    var remBoolean = false
    while (i <= hcidArray.length) {
      var remdocList = Seq[Bson]()
      val hcids = hcidArray.slice(i, staticount)
      ...
      if (doc.get("remQueryExecutor") != null) {
        ...
        for (blk <- doc.get("remQueryExecutor").asInstanceOf[java.util.ArrayList[String]]) {
          if (blk == "remMatchBlockAll") {
            remdocList = remdocList :+ Document.parse(doc.getString("remMatchBlockAll").replaceAll("<INDIC_ID>", concatString(args(4))).replaceAll("<GRP_ID>", concatString(args(5))).replace("<ID_KEY>", concatString(hcids)))
          } else {
        ...
        println("Else Query")
        println(doc.getString("remMatchBlockAll").replaceAll("<INDIC_ID>", concatString(args(4))).replaceAll("<GRP_ID>", concatString(args(5))).replace("<ID_KEY>", concatString(hcids)))
        println(doc.getString("projectBlk"))
      }
      val dateFmt = doc.getString("dateFormatinCSV")
      println("Rem Dep Query Finished  ---------------------------------------------------------")
      val remOutput = MongoSpark.load(sparkContext, readConfig).withPipeline(remdocList)
//      println(" count  - " + remOutput.count())
     
      if (remOutput != null && remOutput.take(1).length > 0) {
        println("Came inside remOutput")
        remBoolean = true
        var remDF = remOutput.toDS[SchemaData]()
        checkAndDeleteFolder(sparkContext, filepath)
        val destinationPath = filepath + "/mergedFile"
        var remDF1:DataFrame = null
        var flag = true
        ...
        if (doc.getString("dateField") != null) {
          ...
          for (dtField <- dateField) {
            if(flag){
              remDF1 = remDF.withColumn(dtField, when(col(dtField).isNull, "").otherwise(date_format(col(dtField), dateFmt)))
              flag = false
            }else{
              remDF1 = remDF1.withColumn(dtField, when(col(dtField).isNull, "").otherwise(date_format(col(dtField), dateFmt)))
            }
          }
        }

        remDF1.write.json(filepath)
        Utils.merge(sparkContext, filepath, destinationPath, "Sample_" + i, "", "")
      }
    }

        val destfilepath = filepath + "/mergedFile/Sample*"    
        println("remBoolean   "+ remBoolean)
        var remData:DataFrame = null   

      if(remBoolean){
        remData = sqlContext.read.json(destfilepath)
        remData.createOrReplaceTempView("remaining_cntrcts")
      }
    
    if (remBoolean && remData != null && remData.take(1).length > 0   && outputMainBool) {
...
//    val addressUpdateQuery = doc.getString("sqlQueryDF2")
    ...
    if (updatedMainDF != null && updatedMainDF.take(1).length > 0) {
      ...
      val depAddQry = doc.getString("sqlQueryDF2")
      val depQry = depAddQry.replaceAll("<TAB1>", "AddressUpdate").replaceAll("<COL_NAME>", concatString("tab3.", doc.getString("TColumnHeaders"), " as "))
      val depDF = sqlContext.sql(depQry)
//      val depDF = sqlContext.sql("Select tab1.* from AddressUpdate tab1 where tab1.Indicator='DEP' ")
  ...
  def processFileGeneration(qryDataFrame1: DataFrame, doc: Document, args: Array[String], sparkContext: SparkContext, sqlContext: SparkSession, appConf: Config) {
    ...
    var qryDataFrame: DataFrame = qryDataFrame1
    qryDataFrame.persist()
    ...
    if (qryDataFrame != null) { // && qryDataFrame.take(1).length > 0) {
      ...
      if (doc.getString("dateField") != null) {
        val dateField = doc.getString("dateField").split(",")
        for (dtField <- dateField) {
          qryDataFrame = qryDataFrame.withColumn(dtField, when(col(dtField).isNull, "").otherwise(date_format(col(dtField), dateFmt)))
        }
      }
      ...
      if (generateFileBasedOn != null) {
        ...
        for (grpBy <- grpByFld) {
          val finalDataFrame = qryDataFrame.filter(qryDataFrame.col(generateFileBasedOn).equalTo(grpBy))
        ...
        var header = ""; 
        var trailer = ""
        if(doc.getString("header")!=null){
          header = doc.getString("header").replaceAll("<DATE>", dateFolder)
        }else{
          header = qryDataFrame.columns.mkString(doc.getString("delimiter"))
        }
         
        if(doc.getString("trailer")!=null){
          trailer = doc.getString("trailer").replaceAll("<REC_CNT>", numFormat.format(qryDataFrame.count()))
        }else{
          trailer = "T|"+numFormat.format(qryDataFrame.count())
        }
        generateFile(qryDataFrame, path, mergeLocation, fileName + ".csv", finalLoc, doc.getString("delimiter"), sparkContext,header,trailer)
      }
    }
  }
  ...
  def fixColumns(dataFrame: DataFrame, doc: Document, sqlContext: SparkSession, conType: String, columnHeadersName: String) = {
    ...
    println("fixColumns   "+ conType )
//    if (dataFrame.take(1).length > 0) {
    if (!dataFrame.limit(1).collect().isEmpty) {
      println("fixColumns   inside ")
